...
function levenshtein(a: string, b: string) {
    const A = normForMatch(a);
    const B = normForMatch(b);
    const n = A.length;
    const m = B.length;
    if (!n) return m;
    if (!m) return n;
...
function bestMatchRegionName(raw: string) {
    const q = normForMatch(raw);

    // まず完全一致/部分一致を優先（速い＆強い）
    if (q) {
        const exact = PLATE_REGIONS.find((r) => normForMatch(r.name) === q);
        if (exact) return exact.name;

        const includes = PLATE_REGIONS.find((r) => normForMatch(r.name).includes(q));
    ...
    for (const r of PLATE_REGIONS) {
        const d = levenshtein(q, r.name);
        if (!best || d < best.dist) best = { name: r.name, dist: d };
    }

    // 近すぎないと採用しない（誤爆防止）
    // 例: 2〜4文字地域名が多いので「距離2まで」くらいが無難
    if (!best) return "";
    return best.dist <= 2 ? best.name : "";
}
...
function parsePlateText(raw: string): OcrResult {
    ...
    const regionRaw = regionMatch ? regionMatch[0] : "";
    const regionName = bestMatchRegionName(regionRaw);
