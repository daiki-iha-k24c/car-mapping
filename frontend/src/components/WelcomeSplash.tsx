...
export default function WelcomeSplash({ totalMs = 6200 }: { totalMs?: number }) {
  ...
  const t = useMemo(() => {
    // ゆっくり：type 2.2s / swap 0.2 / ballize 0.35 / drop 1.1 / puff 1.0 / out 残り
    const type = 2200;
    const swap = 220;
    const ballize = 350;
    const drop = 1100;
    const puff = 1000;
    const out = Math.max(400, totalMs - type - swap - ballize - drop - puff);
    return { type, swap, ballize, drop, puff, out };
  }, [totalMs]);
  ...
  const placeSplashAtBall = () => {
    const root = rootRef.current;
    const ball = ballRef.current;
    const splash = splashRef.current;
    if (!root || !ball || !splash) return;
  ...
  useEffect(() => {
    ...
    const s4 = window.setTimeout(() => {
      // puff直前に、ballの現在位置を起点にする
      placeSplashAtBall();
      setPhase("puff");
    }, t.type + t.swap + t.ballize + t.drop);
    const s5 = window.setTimeout(() => setPhase("out"), t.type + t.swap + t.ballize + t.drop + t.puff);
  ...
  return (
    <div ref={rootRef} className={`ws-root ws-${phase}`} aria-label="起動中">
      ...
      <span ref={splashRef} className="ws-splash" aria-hidden="true" />
